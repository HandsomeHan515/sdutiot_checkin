<?php include 'head.php';?>
	 ...
	 <ul class="nav navbar-nav">
			  ...
			  <li class="active"><a href="result.php">近期记录</a></li>
...
$result = mysql_query("SELECT * FROM checkin order by date desc,start desc LIMIT 200");
...
while($row = mysql_fetch_array($result))
  {
  echo "<tr><td><a href='name.php?name=".$row['id']."'>".$row['id']."</a></td><td>".$row['start']."</td><td>".$row['end']."</td><td>".(($row['stay_time']/60)%10)."小时".($row['stay_time']%60)."分钟</td><td>".$row['duration']."</td><td>".$row['date']."</td><td>".$row['ip']."</td></tr>";
  }
echo "</tbody></table>";
echo "</div>";
echo "<div class='col-md-2'></div>";
